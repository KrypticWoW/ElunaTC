...
void WorldDatabaseConnection::DoPrepareStatements()
{
    ...
    PrepareStatement(WORLD_SEL_GAMEEVENT_INFORMATION, "SELECT `ID`, `Name`, `Start_MapID`, `Start_X`, `Start_Y`, `Start_Z`, `Start_O`, `Flags` + 0, `CreatureGUIDs`, `MinCharacters` FROM custom.gameevent_information WHERE Active = 0 ORDER BY ID", CONNECTION_SYNCH);
}
