...
void CharacterDatabaseConnection::DoPrepareStatements()
{
    ...
    PrepareStatement(CUSTOM_CHAR_SEL_INVENTORY, "SELECT ii.`itemEntry`, it.`name` AS itemName, ii.`count`, ch.`name` AS CharName, ii.`enchantments`, ii.`randomPropertyId`, ch.`level`, ch.`account`, ii.`owner_guid` FROM item_instance ii INNER JOIN characters ch ON ch.`guid` = ii.`owner_guid` INNER JOIN world.`item_template` it ON ii.`itemEntry` = it.`entry` WHERE LOWER(ch.`name`) LIKE LOWER(?) ORDER BY ii.`itemEntry`;", CONNECTION_SYNCH);
