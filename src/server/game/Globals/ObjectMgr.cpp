...
void ObjectMgr::LoadCreatureTemplates()
{
    ...
    QueryResult result = WorldDatabase.Query(
        ...
        " LEFT JOIN creature_custom_experience cce ON ct.entry = cce.CreatureId"
...
void ObjectMgr::LoadCreatureTemplate(Field* fields)
{
    ...
    creatureTemplate.NeckExp               = fields[65].GetUInt32();
}
...
void ObjectMgr::LoadItemTemplates()
{
    ...
    QueryResult customResult = WorldDatabase.Query("select (select count(*) from item_template) as count1, (select count(*) from item_template_custom) as count2");
    ...
    if (customResult)
    {
        ...
        uint64 ItemTemplateCount = uint64(fields[0].GetUInt32() + fields[1].GetUInt32());

        _itemTemplateStore.reserve(ItemTemplateCount);
    }
    else
    {
        TC_LOG_INFO("server.loading", ">> Unable to get item template count.");
        return;
    }
...
void ObjectMgr::LoadCustomItemTemplates()
{
    ...
    QueryResult result = WorldDatabase.Query("SELECT entry, class, subclass, SoundOverrideSubclass, name, displayid, Quality, Flags, FlagsExtra, BuyCount, BuyPrice, SellPrice, InventoryType, "
        ...
        "GemProperties, RequiredDisenchantSkill, ArmorDamageModifier, duration, ItemLimitCategory, HolidayId, ScriptName, DisenchantID, "
        //                                           134        135            136
        "FoodType, minMoneyLoot, maxMoneyLoot, flagsCustom FROM item_template_custom");

    if (!result)
    {
        TC_LOG_INFO("server.loading", ">> Loaded 0 item templates. DB table `item_template_custom` is empty.");
        return;
    }
    ...
    TC_LOG_INFO("server.loading", ">> Loaded " SZFMTD " custom item templates in %u ms", _itemTemplateStore.size(), GetMSTimeDiffToNow(oldMSTime));
}

void ObjectMgr::UpdateCustomItemTemplate(ItemTemplate& temp, uint32 entry)
{
    _itemTemplateStore[entry] = temp;
    temp._LoadTotalAP();
}
