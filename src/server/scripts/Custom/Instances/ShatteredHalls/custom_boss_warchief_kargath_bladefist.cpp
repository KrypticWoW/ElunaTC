...
#include "custom_shattered_halls.h"
...
float CustomAssassEntrance[3] = { 275.136f, -84.29f, 2.3f  }; // y -8
float CustomAssassExit[3]     = { 184.233f, -84.29f, 2.3f  }; // y -8
float CustomAddsEntrance[3]   = { 306.036f, -84.29f, 1.93f };

class custom_boss_warchief_kargath_bladefist : public CreatureScript
{
    public:
        custom_boss_warchief_kargath_bladefist() : CreatureScript("custom_boss_warchief_kargath_bladefist") { }

        struct custom_boss_warchief_kargath_bladefistAI : public BossAI
        {
            custom_boss_warchief_kargath_bladefistAI(Creature* creature) : BossAI(creature, DATA_KARGATH)
            {
                Initialize();
                target_num = 0;
            }
            ...
            void SpawnAssassin()
            {
                me->SummonCreature(NPC_SHATTERED_ASSASSIN, CustomAssassEntrance[0], CustomAssassEntrance[1]+8, CustomAssassEntrance[2], 0, TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT, 30s);
                me->SummonCreature(NPC_SHATTERED_ASSASSIN, CustomAssassEntrance[0], CustomAssassEntrance[1]-8, CustomAssassEntrance[2], 0, TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT, 30s);
                me->SummonCreature(NPC_SHATTERED_ASSASSIN, CustomAssassExit[0], CustomAssassExit[1]+8, CustomAssassExit[2], 0, TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT, 30s);
                me->SummonCreature(NPC_SHATTERED_ASSASSIN, CustomAssassExit[0], CustomAssassExit[1]-8, CustomAssassExit[2], 0, TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT, 30s);
            }
                ...
                else
                {
                    ...
                    if (Summon_Assistant_Timer <= diff)
                    {
                        for (uint8 i = 0; i < summoned; ++i)
                        {
                            ...
                            {
                                case 0:
                                    me->SummonCreature(NPC_HEARTHEN_GUARD, CustomAddsEntrance[0], CustomAddsEntrance[1], CustomAddsEntrance[2], 0, TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT, 30s);
                                    break;
                                case 1:
                                    me->SummonCreature(NPC_SHARPSHOOTER_GUARD, CustomAddsEntrance[0], CustomAddsEntrance[1], CustomAddsEntrance[2], 0, TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT, 30s);
                                    break;
                                case 2:
                                    me->SummonCreature(NPC_REAVER_GUARD, CustomAddsEntrance[0], CustomAddsEntrance[1], CustomAddsEntrance[2], 0, TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT, 30s);
                                    break;
                            }
                        }
                        if (urand(0, 9) < 2)
        ...
        {
            return GetCustomShatteredHallsAI<custom_boss_warchief_kargath_bladefistAI>(creature);
        }
};

void AddSC_custom_boss_warchief_kargath_bladefist()
{
    new custom_boss_warchief_kargath_bladefist();
}
