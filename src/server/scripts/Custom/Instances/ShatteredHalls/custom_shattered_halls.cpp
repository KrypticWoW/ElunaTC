...
Position const CustomExecutioner = { 152.8524f, -83.63912f, 2.021005f, 0.06981317f };

class custom_at_nethekurse_exit : public AreaTriggerScript
{
    public:
        custom_at_nethekurse_exit() : AreaTriggerScript("custom_at_nethekurse_exit") { };
        ...
        bool OnTrigger(Player* player, AreaTriggerEntry const*) override
        {
            if (InstanceScript* is = player->GetInstanceScript())
            {
                if (is->instance->IsHeroic())
                {
                    ...
                    is->instance->LoadGrid(CustomExecutioner.GetPositionX(), CustomExecutioner.GetPositionY());
                    if (Creature* kargath = ObjectAccessor::GetCreature(*player, is->GetGuidData(NPC_KARGATH_BLADEFIST)))
                    {
                        ...
                        {
                            executioner = kargath->SummonCreature(NPC_SHATTERED_EXECUTIONER, CustomExecutioner);
...
class custom_boss_shattered_executioner : public CreatureScript
{
    public:
        custom_boss_shattered_executioner() : CreatureScript("custom_boss_shattered_executioner") { }

        struct custom_boss_shattered_executionerAI : public BossAI
        {
            custom_boss_shattered_executionerAI(Creature* creature) : BossAI(creature, DATA_SHATTERED_EXECUTIONER)
            {
                Initialize();
            };
        ...
        {
            return GetCustomShatteredHallsAI<custom_boss_shattered_executionerAI>(creature);
        }
};
