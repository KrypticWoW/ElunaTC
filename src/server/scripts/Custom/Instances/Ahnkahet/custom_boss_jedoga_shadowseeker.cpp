...
enum JedogaSpells
{
    SPELL_RANDOM_LIGHTNING_VISUAL = 102019,
    ...
    SPELL_SACRIFICE_BEAM          = 102039,

    //Jedoga Controller
    SPELL_BEAM_VISUAL_JEDOGA      = 102035,
...
struct custom_boss_jedoga_shadowseeker : public BossAI
{
    custom_boss_jedoga_shadowseeker(Creature* creature) : BossAI(creature, DATA_JEDOGA_SHADOWSEEKER), _volunteerWork(true) { }
...
struct custom_npc_twilight_volunteer : public ScriptedAI
{
    custom_npc_twilight_volunteer(Creature* creature) : ScriptedAI(creature), _instance(creature->GetInstanceScript())
    {
        me->SetReactState(REACT_PASSIVE);
    }
...
// 102018 - Random Lightning Visual Effect
class custom_spell_random_lightning_visual_effect : public SpellScript
{
    PrepareSpellScript(custom_spell_random_lightning_visual_effect);
    ...
    {
        OnDestinationTargetSelect += SpellDestinationTargetSelectFn(custom_spell_random_lightning_visual_effect::ModDestHeight, EFFECT_0, TARGET_DEST_CASTER_RANDOM);
    }
...
{
    RegisterCustomAhnKahetCreatureAI(custom_boss_jedoga_shadowseeker);
    RegisterCustomAhnKahetCreatureAI(custom_npc_twilight_volunteer);
    RegisterSpellScript(custom_spell_random_lightning_visual_effect);
}
